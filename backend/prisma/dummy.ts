...
async function main() {
  ...
  await prisma.academicCalendarDay.createMany({
    data: [
      ...
      { date: new Date('2026-07-13'), type: 'Exam', label: 'Unit Test 1 - Physics' },
    ],
    skipDuplicates: true,
  });
    ...
    for (let i = 1; i <= count; i++) {
      ...
      const student = await prisma.user.create({
        data: {
          ...
          details: { create: { name: `Student ${puYear}-${i}`, profilePic: '' } },
        },
      });
  ...
  console.log(`  Admin login:   authId=nnpuadmin1        password=${DEFAULT_PASSWORD}`);
  console.log(`  Teacher login: authId=nnputeacher1..4    password=${DEFAULT_PASSWORD}`);
  console.log(`  Student login: authId=nnpu1sb26ka1..5 / nnpu2sb26ka1..5   password=${DEFAULT_PASSWORD}`);
}
