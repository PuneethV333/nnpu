...
import { LANG_CODE, STREAM_CODE } from './helper/helper';
...
export class OnboardingService {
  ...
  async resolveSection(tx: Prisma.TransactionClient, dto: CreateStudentDto) {
    ...
    if (!combination) {
      throw new NotFoundException(`Combination "${dto.subjectCode}" not found`);
    }
    ...
    const section = await tx.section.findUnique({
      where: {
        classId_combinationId_language_session_academicYearId: {
      ...
      include: { class: true, combination: true, academicYear: true },
    });

    if (!section) {
      throw new NotFoundException(
        `No section exists for class ${dto.classYear}, subject ${dto.subjectCode}, language ${dto.language}, session ${dto.session}, academic year ${academicYear.label}`,
      );
    }
  ...
  async createStudent(dto: CreateStudentDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const streamCode = STREAM_CODE[section.combination.stream];
      const comboCode = section.combination.idCode;
      const joinYear2 = section.academicYear.startDate
        .getFullYear()
        .toString()
        .slice(-2);
      const langCode = LANG_CODE[section.language];
